...
SHOPIFY_STORE = "dj4qxk-yt"
API_KEY = "5dd52f10564583d30a3a0eccfbb47bb9"
PASSWORD = "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"
...
class ShopifyETLPipeline:
    ...
    def get_last_sync_timestamp(self):
        """Same as your prior logic for orders only."""
        query = f"""
    ...
    def update_sync_timestamp(self, records_processed):
        """Same as your prior logic."""
        query = f"""
        MERGE INTO `{self.control_table}` T
        USING (
          SELECT 'orders' AS table_name, CURRENT_TIMESTAMP() AS last_sync_timestamp, {records_processed} AS records_processed
        ) S
        ON T.table_name = S.table_name
        WHEN MATCHED THEN
          UPDATE SET T.last_sync_timestamp = S.last_sync_timestamp,
                     T.records_processed = S.records_processed
        WHEN NOT MATCHED THEN
          INSERT (table_name, last_sync_timestamp, records_processed)
          VALUES (S.table_name, S.last_sync_timestamp, S.records_processed)
        """
    ...
    def fetch_shopify_data(self, start_date=None, test_mode=False, max_pages=5000, max_retries=5):
        ...
        while next_url and page_count <= max_pages:
            ...
            for attempt in range(max_retries):
                try:
                    if page_count == 1:
                        ...
                        # For subsequent pages, use the next_url directly without adding params
                        response = requests.get(
                            next_url,
                            auth=(API_KEY, PASSWORD),
                            timeout=30
            ...
            for order in orders:
                ...
                # 3) Customers - rest of processing as before
                # ...
    ...
    def verify_unique_records(self, table_name):
        """Verify that there are no duplicates in the final table based on unique keys."""
        unique_cols = UNIQUE_KEYS[table_name]
    ...
    def execute(self, force_full_load=False, test_mode=False, max_pages=5000):
        """Main execution function (Load from GCS => single MERGE => TRUNCATE)."""
        ...
        # 4) Update sync timestamp if desired
